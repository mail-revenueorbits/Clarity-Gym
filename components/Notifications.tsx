    ...
    <div className="space-y-6 max-w-[1400px] mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 md:gap-4 mb-2">
        <div>
          ...
          <p className="text-slate-400 text-xs md:text-sm font-medium mt-0.5 md:mt-1">Manage SMS broadcasts and logs</p>
        </div>
      </div>

      {/* Top Stats Overview */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 border border-slate-100 shadow-sm relative overflow-hidden group flex items-center justify-between">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <p className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-wider">SMS Credits</p>
              <h3 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight">{credits.toLocaleString()}</h3>
            </div>
          </div>
          <button className="text-[10px] md:text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-2 rounded-lg transition-colors uppercase tracking-wider">
            Top Up
          </button>
        </div>
        <div className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="flex items-center gap-3 md:gap-4">
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center">
              <Users className="w-5 h-5 md:w-6 md:h-6" />
            </div>
            <div>
              <p className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-wider">Active Members</p>
              <h3 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight">{activeMembers.length}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 flex items-center p-2 bg-slate-50/50">
          <button
            ...
            className={`flex-1 py-2.5 md:py-3 text-xs md:text-sm font-bold rounded-xl transition-colors ${
              viewTab === 'compose' ? 'bg-white shadow-sm text-red-600 border border-slate-100' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 md:py-3 text-xs md:text-sm font-bold rounded-xl transition-colors ${
              viewTab === 'history' ? 'bg-white shadow-sm text-red-600 border border-slate-100' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            <div className="flex items-center justify-center gap-2">
          ...
          {viewTab === 'compose' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4 md:space-y-6">
                <div>
                  <h2 className="text-base md:text-lg font-bold text-slate-900">1. Select Audience</h2>
                  <p className="text-xs md:text-sm text-slate-500">Define who should receive this message.</p>
                </div>

                {/* Unified Selection Interface */}
                <div className="bg-slate-50 p-4 md:p-5 rounded-2xl border border-slate-100 space-y-4">
                  {/* Demographics Row */}
              ...
              <div className="space-y-4 md:space-y-6">
                <div>
                  <h2 className="text-base md:text-lg font-bold text-slate-900">2. Compose & Send</h2>
                  <p className="text-xs md:text-sm text-slate-500">Craft your message and dispatch it.</p>
                </div>

                <div className="bg-slate-50 p-4 md:p-5 rounded-2xl border border-slate-100 md:h-[calc(100%-4.5rem)] flex flex-col">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                  ...
                  <table className="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                      <tr className="bg-slate-50/50 border-b border-slate-100">
                        <th className="px-6 py-4 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider">Status & Date</th>
                        <th className="px-6 py-4 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider">Type</th>
                        <th className="px-6 py-4 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider w-1/2">Message Content</th>
                        <th className="px-6 py-4 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider text-right">Recipients</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {currentLogs.length === 0 ? (
                        ...
                        currentLogs.map(log => (
                          <tr key={log.id} onClick={() => setSelectedLog(log)} className="hover:bg-slate-50/50 transition-colors cursor-pointer group">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center gap-2 mb-0.5">
                                {log.status === 'sent' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />}
                                <span className={`text-sm font-bold capitalize ${log.status === 'sent' ? 'text-emerald-700' : 'text-amber-600'}`}>
                                  {log.status}
                                </span>
                              </div>
                              <span className="text-[10px] text-slate-400 font-medium">{getFormattedBsDate(log.timestamp)}</span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4">
                              <p className="text-slate-700 text-sm font-medium line-clamp-2 group-hover:text-red-700 transition-colors">{log.message}</p>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <div className="flex items-center justify-end gap-2">
                                <span className="font-bold text-slate-800 text-sm">{log.recipientsCount}</span>
                ...
                <div className="md:hidden divide-y divide-slate-50">
                  {currentLogs.length === 0 ? (
                    <div className="py-12 text-center text-slate-400 text-sm font-medium">No notifications match your current filter.</div>
                  ) : (
                    currentLogs.map(log => (
                      <div key={log.id} onClick={() => setSelectedLog(log)} className="p-4 hover:bg-slate-50/50 cursor-pointer">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <div className="flex items-center gap-1.5 mb-1">
                              {log.status === 'sent' && <CheckCircle2 className="w-3 h-3 text-emerald-500" />}
                              <span className={`text-xs font-bold capitalize ${log.status === 'sent' ? 'text-emerald-700' : 'text-amber-600'}`}>
                                {log.status}
                              </span>
                            </div>
                            <span className="text-[10px] text-slate-400 font-medium">{getFormattedBsDate(log.timestamp)}</span>
                          </div>
                          <span className={`px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider rounded-md border ${
                            log.type === 'promotion' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                            log.type === 'reminder' ? 'bg-orange-50 text-orange-700 border-orange-200' :
                            'bg-red-50 text-red-700 border-red-200'
                          }`}>
                        ...
                        <p className="text-sm font-medium text-slate-700 line-clamp-2 mb-2">{log.message}</p>
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50">
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Recipients</span>
                           <span className="text-xs font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded">{log.recipientsCount}</span>
                        </div>
                      </div>
                    ))
                  )}
